    ...
    <div className="rounded-lg border border-gray-200 bg-white">
      ...
      {isOpen && (
        <div className="border-t border-gray-100 p-4 space-y-3">
          ...
          {warnings.length > 0 && (
            <div className="space-y-2">
              <h4 className="text-xs font-semibold uppercase tracking-wider text-amber-600">Warnings</h4>
              {warnings.map((flag) => (
                <div key={flag.id} className="rounded-md border border-amber-200 bg-amber-50 p-3">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <p className="text-sm font-medium text-amber-800">{flag.rule}</p>
                      <p className="mt-0.5 text-sm text-amber-700">{flag.message}</p>
                      {flag.field && (
                        <p className="mt-1 text-xs text-amber-500">Field: {flag.field}</p>
                      )}
                    </div>
                  </div>
                </div>
