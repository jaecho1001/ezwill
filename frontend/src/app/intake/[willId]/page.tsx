      ...
      <div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        <div className="mb-2 flex flex-wrap items-center gap-2 text-xs">
          <span className="font-medium text-gray-700">
            Intake · {willIntakeChapters.length} chapters
          </span>
          ...
          <span className="ml-auto text-gray-500">{overall.pct}% complete</span>
        </div>
      ...
      <div className="grid grid-cols-12 gap-4">
              ...
              <div className="flex items-center justify-end gap-2 pt-1">
                <Link href={`/summary/${willId}`}>
                  <Button disabled={!canReview}>
                    {canReview ? 'Review & continue →' : `${overall.requiredUnanswered} required left`}
                  </Button>
                </Link>
              </div>
            </>
            ...
            <>
              <header className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-gray-500">{chapter.intro}</p>
                  </div>
                </div>
              </header>
              ...
              {visibleQuestions.length === 0 && (
                <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-5 text-center text-sm text-gray-500">
                  Nothing applies to you in this chapter — move on to the next.
                </div>
              )}
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                <span className="ml-auto text-xs text-gray-400">
                  Chapter {chapterIdx + 1} of {willIntakeChapters.length}
                </span>
              </div>
            </>
          )}
        ...
        <aside className="col-span-12 md:col-span-3">
          <div className="sticky top-4 max-h-[calc(100vh-2rem)] overflow-y-auto pb-4">
            ...
            <ExtractedDataSidebar vault={vault} onJumpTo={setChapterIdx} />
          </div>
        </aside>
      </div>
    </div>
